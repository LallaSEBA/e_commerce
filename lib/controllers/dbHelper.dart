...
import '../ressources/globals.dart';
   ...
   loginData(String email, var password) async{
    ...
    try{
        ...
        if(status){
          ...
          shared_token = data["token"]; 
          shared_name  = data["name"]; 
          shared_email = email; 
          await _saveToken();
          msgStatus='';
       }
...
//الطريقة 2
 ...
 Future<bool> logout(String email)async{
  ...
  _saveToken() async {
    final prefs    = await SharedPreferences.getInstance();
    prefs.setString(shared_token, shared_token);
    prefs.setString(shared_name, shared_name);
    prefs.setString(shared_email, shared_email);
  }
 ...
 readToken() async {
    ...
    {
      shared_token = prefs.getString(shared_token ) ?? '';
      shared_name  = prefs.getString(shared_name ) ?? '';
      shared_email = prefs.getString(shared_email ) ?? '';
    }
    else
    {
      shared_token = '';
    ...
    print('read : $shared_token $shared_name $shared_token');
    return shared_token;
  }
